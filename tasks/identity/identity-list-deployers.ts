...
task('identity-list-deployers', 'Will list the active deployers of an identity')
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    const identityDeployerChangedFilter =
      identityContract.filters.IdentityDeployerChanged();
    let identityDeployerChangedEvents = await identityContract.queryFilter(
      identityDeployerChangedFilter
    );
    ...
    for (const e of identityDeployerChangedEvents) {
      if (e.args) {
        ...
        if (identity === taskArgs.identity) {
          ...
          } else if (!skipList.includes(deployer)) {
            skipList.push(deployer);
          }
        }
      }
