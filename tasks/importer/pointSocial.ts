...
task(
  'pointsocial-importer',
  'Will download and upload data to point  pointSocial contract'
)
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    if (taskArgs.action == 'download') {
      ...
      const pointSocial = await hre.ethers.getContractAt(
        'PointSocial',
        contractAddress
      );
      ...
      if (taskArgs.fromPort == undefined || taskArgs.toPort == undefined) {
        console.log(
          'Undefined ports do donwload or upload arweave data, please set --from-port and --to-port parameters.'
        );
        return false;
      }

      https.globalAgent.options.rejectUnauthorized = false;
      const importString = async (
        id: string,
        type: string
      ): Promise<string> => {
        type DataRequest = {
          ...
          if (
            ...
            if (type === 'image') {
              response = await fetch(
                `https://localhost:${taskArgs.fromPort}/_storage/${id}`
              );
              ...
              response = await fetch(
                `https://localhost:${taskArgs.fromPort}/v1/api/storage/getString/${id}`
              );
              if (response.ok) {
                ...
                response = await fetch(
                  `https://localhost:${taskArgs.toPort}/v1/api/storage/putString`,
                  {
                    method: 'POST',
                    headers: {
      ...
      try {
        ...
        for (const post of data.posts) {
          ...
          if (lastProcessedPost > processPostsFrom || processPostsFrom == 0) {
            ...
            const importedContents = await importString(
              post.contents,
              'post-content'
            );
