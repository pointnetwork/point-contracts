...
task('sms-importer', 'Will download and upload data to point identity contract')
  ...
  .addOptionalParam(
    'handle',
    'Which handle to use to discover contract address'
  )
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    if (taskArgs.action == 'download') {
      ...
      while (keepSearchingTweets) {
        try {
          const { from, contents, timestamp, likes } = await sms.getTweet(
            tweetCounter.toString()
          );
          const tweet = {
            from,
            contents,
      ...
      const sms = await hre.ethers.getContractAt(
        contractName,
        taskArgs.contract
      );
      ...
      try {
        ...
        for (const tweet of data.tweets) {
          lastProcessedTweet++;
          if (
            lastProcessedTweet > processTweetsFrom ||
            processTweetsFrom == 0
          ) {
            console.log(
              `Migrating: tweet from ${tweet.from} contents ${tweet.contents}`
            );
            await sms.add(
              tweet.from,
              tweet.contents,
        ...
        console.log(
          'Error on Sms import restart the process to pick-up from last processed item.'
        );
        return false;
      }
    }
