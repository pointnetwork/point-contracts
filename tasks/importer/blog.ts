    ...
    if (taskArgs.action == 'download') {
      ...
      try {
        ...
        for (const article of data.articles) {
          ...
          if (
            lastProcessedArticle > processArticlesFrom ||
            processArticlesFrom == 0
          ) {
            console.log(
              `${lastProcessedArticle} Migrating: Blog post from ${article.author} contents ${article.contents}`
            );
        ...
        for (const postId in articleComments) {
          for (const comment of articleComments[postId]) {
            ...
            if (
              ...
              console.log(
                `${lastProcessedComment} Migrating comment from article ${postId} from ${author}`
              );

              await blog.addComment(postId, author, contents, timestamp);
            } else {
        ...
        console.log(
          `Error on Blog import restart the process to pick-up from last processed item ${error}`
        );
        return false;
      }
    }
