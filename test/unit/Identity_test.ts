...
describe('Token identity contract', function () {
  ...
  describe('Testing owner functions', function () {
    it('Validating identity owner', async function () {
      const ownerIdentity = await identityContract.getIdentityByOwner(
        addr1.address
      );
    ...
    it('Validating owner can set IKVvalue', async function () {
      ...
      await identityContract
        .connect(addr1)
        .ikvPut(handle, 'test', shouldBe, 'latest');
      const actual = await identityContract
        .connect(addr1)
        .ikvGet(handle, 'test');
    ...
    it("Validate owner can transfer it's ownership", async function () {
      await expect(
        identityContract
          .connect(addr1)
          .transferIdentityOwnership(handle, addr2.address)
    ...
    it("Validating can't transfer ownership to itself", async function () {
      await expect(
        identityContract
          .connect(addr1)
          .transferIdentityOwnership(handle, addr1.address)
  ...
  describe('Testing helper functions', function () {
    it('Loads identity public key', async function () {
      ...
      const commPublicKey = await identityContract
        .connect(addr1)
    ...
    it('Invalid handle', async function () {
      await expect(
        ...
        'Only alphanumeric characters and an underscore allowed'
      );
    });
  ...
  describe('Testing migrator functions', function () {
    it('Owner can migrate IKV to any user', async function () {
      ...
      await identityContract
        .connect(owner)
        .ikvImportKV(handle, 'migrated-by-owner', shouldBe, 'latest');
      const actual = await identityContract
        .connect(addr1)
        .ikvGet(handle, 'migrated-by-owner');
      expect(actual).to.equal(shouldBe);
    });
  ...
  describe('Testing deployer functions', function () {
    it('Owner can add deployer to its identity', async function () {
      ...
      await expect(
        identityContract
          .connect(addr1)
          .addIdentityDeployer(handle, addr2.address)
    ...
    it("Can't set owner as deployer", async function () {
      await expect(
        identityContract
          .connect(addr1)
          .addIdentityDeployer(handle, addr1.address)
    ...
    it('Owner can remove deployer to their identity', async function () {
      ...
      await expect(
        identityContract
          .connect(addr1)
          .removeIdentityDeployer(handle, addr2.address)
      ).not.to.be.revertedWith('You are not the owner of this identity');
    });
    ...
    it("Can't remove owner as deployer", async function () {
      await expect(
        identityContract
          .connect(addr1)
          .removeIdentityDeployer(handle, addr1.address)
    ...
    it('Can query if an address is a deployer', async function () {
      const isOwnerDeployer = await identityContract
        .connect(addr1)
        .isIdentityDeployer(handle, addr1.address);
      expect(isOwnerDeployer).to.equal(true);

      const isNotDeployer = await identityContract
        .connect(addr1)
      ...
      await identityContract
        .connect(addr1)
        .addIdentityDeployer(handle, addr2.address);
      const isADeployer = await identityContract
        .connect(addr1)
        .isIdentityDeployer(handle, addr2.address);
  ...
  describe('Testing register validation', function () {
    it('Can register with a proper signed msg with free status ', async function () {
      ...
      const flatSig = await addr1.signMessage(messageHashBytes);
    ...
    it("Can't register with signed msg by other than oracle", async function () {
      ...
      // Sign the string message
      const flatSig = await addr2.signMessage(messageHashBytes);
