...
describe("PointSocial contract", function () {
  ...
  let handle: string;
  let postContent =
    "0x9d6b0f937680809a01639ad1ae4770241c7c8a0ded490d2f023669f18c6d744b";
  let postimage =
    "0x5f04837d78fa7a656419f98d73fc1ddaac1bfdfca9a244a2ee128737a186da6e";

  beforeEach(async function () {
    [owner, addr1, addr2, ...addrs] = await ethers.getSigners();
    const factory = (await ethers.getContractFactory(
      "PointSocial"
    )) as PointSocial__factory;
  ...
  describe("Testing migrator functions", function () {
    it("User can add migrator", async function () {
      await pointSocial.addMigrator(addr1.address);

      await expect(pointSocial.addMigrator(addr2.address)).to.be.revertedWith(
        "Access Denied"
      );
    });
    ...
    it("User can add migrator migrator can add post", async function () {
      ...
      await pointSocial
        .connect(addr1)
        .add(1, addr2.address, postContent, postimage, 0, "1646689414");
    ...
    it("Migrator user can add migrator comment", async function () {
      ...
      await pointSocial
        .connect(addr1)
        .addComment(1, 1, addr2.address, postContent, "1646689414");
    ...
    it("Random user can't add migrator comment", async function () {
      ...
      await expect(
        pointSocial.addComment(1, 1, addr2.address, postContent, "1646689414")
      ).to.be.revertedWith("Access Denied");
    });
  ...
  describe("Testing user functions", function () {
    ...
    it("Get paginated posts", async function () {
      ...
      const paginatedPosts = await pointSocial.getPaginatedPostsByOwner(
        owner.address,
        "1",
        "10"
      );
    ...
    it("Add like to posts twice same user", async function () {
      ...
      const postsByOwnerLenght = await pointSocial.getAllPostsByOwnerLength(
        owner.address
      );
      const posts = await pointSocial.getAllPosts();
